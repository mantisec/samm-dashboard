...
router.get('/isecuritysecondgraph',function(req,res){
  function red_1(){
    var defered = q.defer();
    db.query("select sum(status) as score, count(*) as total from  isecurity_red where compliance_section = 'A.17.2.1-Availability of information processing facilities' ",defered.makeNodeResolver());
    return defered.promise;
  }

  function isc_1(){
    var defered = q.defer();
    db.query("select sum(status) as score, count(*) as total from  isecurity_isc where compliance_section = 'A.17.1.1-Planning information security continuity' ",defered.makeNodeResolver());
    return defered.promise;
  }
  function isc_2(){
    var defered = q.defer();
    db.query("select sum(status) as score, count(*) as total from  isecurity_isc where compliance_section = 'A.17.1.2-Implementing information security continuity'",defered.makeNodeResolver());
    return defered.promise;
  }
  function isc_3(){
    var defered = q.defer();
    db.query("select sum(status) as score, count(*) as total from  isecurity_isc where compliance_section = 'A.17.1.3-Verify, review and evaluate information security continuity'",defered.makeNodeResolver());
    return defered.promise;
  }

  q.all([red_1(),
    isc_1(),isc_2(),isc_3()]).then(function(results){




      res.json({success:true,red1:results[0][0][0].score,
        isc1:results[1][0][0].score,
        isc3:results[2][0][0].score,
        isc3:results[3][0][0].score});
   });
});
//****************THIRD GRAPH********************* *//
router.get('/isecuritythirdgraph',function(req,res){
  function red_yes(){
    var defered = q.defer();
    db.query("select count(*) as score from  isecurity_red  where status = 1  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function red_no(){
    var defered = q.defer();
    db.query("select count(*) as score from  isecurity_red  where status = 0  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function isc_yes(){
    var defered = q.defer();
    db.query("select count(*) as score from  isecurity_isc  where status = 1  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function isc_no(){
    var defered = q.defer();
    db.query("select count(*) as score from  isecurity_isc  where status = 0  ",defered.makeNodeResolver());
    return defered.promise;
  }
  q.all([red_yes(),red_no(),
    isc_yes(),isc_no()]).then(function(results){

    red_yes = (results[0][0][0].score/10);
    red_no = (results[1][0][0].score/10);
    isc_yes = (results[2][0][0].score/10);
    isc_no = (results[3][0][0].score/10);
    res.json({success:true, red_yes:red_yes, red_no: red_no,isc_yes: isc_yes,isc_no: isc_no });
  });
});
//****************FIRST GRAPH********************* *//
router.get('/isecurityscores',function(req,res){
  // console.log("hii");
  function fun1(){
    var defered = q.defer();
...
router.get('/isecurityfirstgraph',function(req,res){
  function de_score(){
    var defered = q.defer();
    db.query("select sum(status) as score, count(*) as total from  isecurity_isc where status!=-1 ",defered.makeNodeResolver());
    return defered.promise;
  ...
  function de_total(){
    var defered = q.defer();
    db.query("select count(*) as total from  isecurity_isc ",defered.makeNodeResolver());
    return defered.promise;
  }
  function pe_score(){
    var defered = q.defer();
  ...
  function pe_total(){
    var defered = q.defer();
    db.query("select count(*) as total from  isecurity_red ",defered.makeNodeResolver());
    return defered.promise;
  }
  q.all([de_score(), de_total(),pe_score(),pe_total()]).then(function(results){

    isc = results[0][0][0].score;

    b = results[1][0][0].total;

   v1 = Math.round((isc/b)*100);
   v2=Math.round((results[2][0][0].score/results[3][0][0].total)*100);

     res.json({success:true,isc:v1,red:v2});
   });

});
//****************TABLE********************* *//
router.get('/isecurity_isc_tab', function (req, res) {
  sql="SELECT * FROM isecurity_isc  ORDER BY iso_id ASC ";
...
router.post('/isecurity_isc_getuploaded_files', function(req,res)
{
  sql="SELECT filename FROM `isecurity_isc` WHERE iso_id='"+req.body.id+"'";
 console.log(sql);
  db.query(sql,(err,results)=>
  {
      if(err) throw err;
...
//update
router.put('/isecurity_isc_edit', function (req, res) {
...
if(score=="No")
    ...
    {
        var c='';
        sql="update isecurity_isc  set status="+scname+" where iso_id="+id+"";
        ...
        sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SECURITY-ISC "+id+"','"+question+"','Information Security Aspects of Business Continuity Management','Information security continuit','"+scname+"','"+audit+"',NOW())";
        db.query(sql,(err,results)=>
        {
            if(err) throw err;
    ...
    {
        sql="update isecurity_isc  set comment='"+req.body.comment+"' where iso_id="+id+"";
        sql1="INSERT INTO comment_history VALUES ('"+question+"','','Information Security Aspects of Business Continuity Management','Information security continuit','"+req.body.comment+"','"+audit+"',CURRENT_TIMESTAMP,'"+scname+"',"+id+");"
        sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SECURITY-ISC "+id+"','"+question+"','Information Security Aspects of Business Continuity Management','Information security continuit','"+scname+"','"+audit+"',NOW())";
        db.query(sql,(err,results)=>
        {
            if(err) throw err;
    ...
    {
        sql="update isecurity_isc set status="+scname+",comment='"+req.body.comment+"' where iso_id="+id+"";
        sql1="INSERT INTO comment_history VALUES ('"+question+"','','Information Security Aspects of Business Continuity Management','Information security continuit','"+req.body.comment+"','"+audit+"',CURRENT_TIMESTAMP,'"+scname+"',"+id+");"
        sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SECURITY-ISC "+id+"','"+question+"','Information Security Aspects of Business Continuity Management','Information security continuit','"+scname+"','"+audit+"',NOW())";
        db.query(sql,(err,results)=>
        {
...
router.get('/dovagraph_isecurity', function (req, res) {
  // let d = Date.now();
  let d1 = new Date();
