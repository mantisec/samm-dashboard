...
router.get('/suppliersecondgraph',function(req,res){
  ...
  function isr_3(){
    var defered = Q.defer();
    db.query("select sum(status) as score, count(*) as total from  supplier_isr where compliance_section = 'A.15.1.3-Information and communication technology supply chain' ",defered.makeNodeResolver());
    return defered.promise;
  }

  Q.all([sdm_1(),sdm_2(),
    isr_1(),isr_2(),isr_3()]).then(function(results){




      res.json({success:true,sdm_1:results[0][0][0].score,
        sdm_2:results[1][0][0].score,

        isr_1:results[2][0][0].score,
        isr_2:results[3][0][0].score,
        isr_3:results[4][0][0].score
        });
   });
});
//****************THIRD GRAPH********************* *//
router.get('/supplierthirdgraph',function(req,res){
  function sdm_yes(){
    var defered = Q.defer();
    db.query("select count(*) as score from  supplier_sdm  where status = 1  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function sdm_no(){
    var defered = Q.defer();
    db.query("select count(*) as score from  supplier_sdm  where status = 0  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function isr_yes(){
    var defered = Q.defer();
    db.query("select count(*) as score from  supplier_isr  where status = 1  ",defered.makeNodeResolver());
    return defered.promise;
  }
  function isr_no(){
    var defered = Q.defer();
    db.query("select count(*) as score from  supplier_isr  where status = 0  ",defered.makeNodeResolver());
    return defered.promise;
  }
  Q.all([sdm_yes(),sdm_no(),
    isr_yes(),isr_no()]).then(function(results){

    sdm_yes = (results[0][0][0].score/10);
    sdm_no = (results[1][0][0].score/10);
    isr_yes = (results[2][0][0].score/10);
    isr_no = (results[3][0][0].score/10);
    res.json({success:true, sdm_yes:sdm_yes, sdm_no: sdm_no,isr_yes: isr_yes,isr_no: isr_no });
  });
});
//****************FIRST GRAPH********************* *//
router.get('/supplierscores',function(req,res){
  // console.log("hii");
  function fun1(){
    var defered = Q.defer();
...
router.get('/supplierfirstgraph',function(req,res){
  function isr_score(){
    var defered = Q.defer();
    db.query("select sum(status) as score, count(*) as total from  supplier_isr where status!=-1 ",defered.makeNodeResolver());
    return defered.promise;
  ...
  function isr_total(){
    var defered = Q.defer();
    db.query("select count(*) as total from  supplier_isr ",defered.makeNodeResolver());
    return defered.promise;
  }
  function sdm_score(){
    var defered = Q.defer();
  ...
  function sdm_total(){
    var defered = Q.defer();
    db.query("select count(*) as total from  supplier_sdm ",defered.makeNodeResolver());
    return defered.promise;
  }
  Q.all([isr_score(), isr_total(),sdm_score(),sdm_total()]).then(function(results){
   ...
   v2=Math.round((results[2][0][0].score/results[3][0][0].total)*100);
     ...
     res.json({success:true,isr:v1,sdm:v2});
   });

});
//****************TABLE********************* *//
router.get('/supplier_isr_tab', function (req, res) {
  sql="SELECT * FROM supplier_isr  ORDER BY iso_id ASC ";
...
router.post('/supplier_isr_getuploaded_files', function(req,res)
{
  sql="SELECT filename FROM `supplier_isr` WHERE iso_id='"+req.body.id+"'";
 console.log(sql);
  db.query(sql,(err,results)=>
  {
      if(err) throw err;
...
//update
router.put('/supplier_isr_edit', function (req, res) {
...
{
    var c='';
    sql="update supplier_isr set status="+scname+" where iso_id="+id+"";
    ...
    sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SUP-ISR "+id+"','"+question+"','Supplier Relationships','Information security in supplier relationships','"+scname+"','"+audit+"',NOW())";
    db.query(sql,(err,results)=>
    {
        if(err) throw err;
...
{
    sql="update supplier_isr set comment='"+req.body.comment+"' where iso_id="+id+"";
    sql1="INSERT INTO comment_history VALUES ('"+question+"','','Supplier Relationships','Information security in supplier relationships','"+req.body.comment+"','"+audit+"',CURRENT_TIMESTAMP,'"+scname+"',"+id+");"
    sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SUP-ISR "+id+"','"+question+"','Supplier Relationships','Information security in supplier relationships','"+scname+"','"+audit+"',NOW())";
    db.query(sql,(err,results)=>
    {
        if(err) throw err;
...
{
    sql="update supplier_isr set status="+scname+",comment='"+req.body.comment+"' where iso_id="+id+"";
    sql1="INSERT INTO comment_history VALUES ('"+question+"','','Supplier Relationships','Information security in supplier relationships','"+req.body.comment+"','"+audit+"',CURRENT_TIMESTAMP,'"+scname+"',"+id+");"
    sql2="Insert into score_history(qoid,qoname,question,groupname, secpractice,score,uname,created) values("+id+",'SUP-ISR "+id+"','"+question+"','Supplier Relationships','Information security in supplier relationships','"+scname+"','"+audit+"',NOW())";
    db.query(sql,(err,results)=>
    {
...
router.get('/dovagraph_sup', function (req, res) {
  // let d = Date.now();
  let d1 = new Date();
